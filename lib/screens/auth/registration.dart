...
import 'package:haxuvina/custom/google_recaptcha.dart';
import 'package:haxuvina/custom/input_decorations.dart';
import 'package:haxuvina/custom/intl_phone_input.dart';
...
import 'package:haxuvina/screens/common_webview_screen.dart';
...
class _RegistrationState extends State<Registration> {
  ...
  @override
  void initState() {
    //on Splash Screen hide status bar
  ...
  onPressSignUp() async {
    ...
    try {
      ...
      // Validate input
      if (name.isEmpty) {
        throw Exception(AppLocalizations.of(context)!.enter_your_name);
      }

      if (_register_by == 'email') {
        if (email.isEmpty || !isEmail(email)) {
          throw Exception(AppLocalizations.of(context)!.enter_email);
        }
      } else {
        if (phone.isEmpty) {
          throw Exception(AppLocalizations.of(context)!.enter_phone_number);
        }
      }

      if (password.isEmpty) {
        throw Exception(AppLocalizations.of(context)!.enter_password);
      }

      if (passwordConfirm.isEmpty) {
        throw Exception(AppLocalizations.of(context)!.confirm_your_password);
      }

      if (password.length < 6) {
        throw Exception(AppLocalizations.of(context)!.password_must_contain_at_least_6_characters);
      }
      ...
      final signupResponse = await AuthRepository().getSignupResponse(
        ...
        _register_by,
        googleRecaptchaKey,
        referralCode, // ✅ thêm dòng này
      );

      if (signupResponse.result != true) {
        final msg = signupResponse.message;
        String message;
        if (msg is List) {
          message = msg.join('\n');
        } else {
      ...
      // Redirect
      if (_register_by == "phone" && otp_addon_installed.$) {
        GoRouter.of(context).push('/otp', extra: {
          'phone': phone,
          'mode': 'register_phone',
        });
      } else if (_register_by == "email" && mail_verification_status.$) {
        GoRouter.of(context).push('/otp', extra: {
          'phone': email,
          'mode': 'register_email',
        });
      ...
      ToastComponent.showDialog(e.toString().replaceFirst('Exception: ', ''));
    } finally {
      Loading.close();
    }
  }
