...
class Coupons extends StatefulWidget {
  final bool selectMode; // ✅ thêm

  const Coupons({Key? key, this.selectMode = false}) : super(key: key);
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        if (widget.selectMode) {
                          Navigator.pop(context, _couponsList[index].code);
                        } else {
                          Clipboard.setData(ClipboardData(text: _couponsList[index].code!))
                              .then((_) {
  ...
  Widget buildCouponHeader(int index) {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 5),
        Text(
          _couponsList[index].discount_type == "percent"
              ? "${LangText(context).local.off} ${_couponsList[index].discount}%"
              : "${LangText(context).local.off} ${convertPrice(_couponsList[index].discount.toString())}",
          style: TextStyle(
            color: Colors.white,
            fontSize: 21,
            fontWeight: FontWeight.bold,
