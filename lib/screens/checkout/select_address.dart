...
import 'package:haxuvina/presenter/select_address_provider.dart';
...
class _SelectAddressState extends State<SelectAddress> {
  double mWidth = 0;
  double mHeight = 0;
  late SelectAddressProvider _selectAddressProvider;

  @override
  void initState() {
    super.initState();

    // ✅ Gán Provider một lần, không bị reset
    _selectAddressProvider = SelectAddressProvider();
    _selectAddressProvider.init(context);

    // ✅ Gọi lại sau khi tạo tài khoản
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (notificationShowType.$ == "password_account_created") {
        print("⚡ Đồng bộ lại trạng thái login sau khi tạo tài khoản");

        AuthHelper().fetch_and_set().then((_) {
          _selectAddressProvider.fetchShippingAddressList(context);
        });
      }
    });
  }
  ...
  Widget buildShippingInfoList(selectAddressProvider, BuildContext context) {
    ...
    if (!selectAddressProvider.faceData) {
      return ShimmerHelper().buildListShimmer(item_count: 5, item_height: 100.0);
    }

    // Đã có dữ liệu, nhưng danh sách rỗng
    if (selectAddressProvider.shippingAddressList.isEmpty) {
      return Container(
        height: 100,
        child: Center(
          child: Text(
