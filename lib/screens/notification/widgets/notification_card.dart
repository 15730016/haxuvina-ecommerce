...
class _NotificationListCardState extends State<NotificationListCard> {
  _onTap(context) {
    ...
    if (widget.type == "App\\Notifications\\CustomNotification") {
      final rawLink = widget.link?.toString(); // ✅ luôn ép sang String
      if (rawLink != null && rawLink.isNotEmpty) {
        try {
          final relativePath = rawLink.split(AppConfig.DOMAIN_PATH).last;
          final cleanPath = relativePath.startsWith('/') ? relativePath : '/$relativePath';
          context.go(cleanPath);
        } catch (e) {
          debugPrint("⚠️ Lỗi khi xử lý link trong Notification: $e");
        }
      } else {
        debugPrint("⚠️ Link không hợp lệ hoặc null: $rawLink");
      }
    }
